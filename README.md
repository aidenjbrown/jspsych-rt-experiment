...
- **Memory and attention study (naturalistic variant)** ([live](https://aidenjbrown.github.io/jspsych-rt-experiment/memory-shibboleth-naturalistic.html) · `memory-shibboleth-naturalistic.html`) — identical to the task above, except the participant ID is shown with no mention that it'll be tested later. Not yet broadcast; held for a separate data collection window so the two conditions aren't mixed.
